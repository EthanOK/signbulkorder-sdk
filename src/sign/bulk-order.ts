...
export class BulkOrder<T extends OrderParameters = OrderParameters> {
  ...
  private eip712BulkOrderType: EIP712TypeDefinitions;
  ...
  constructor(
    ...
    this.eip712BulkOrderType = eip712BulkOrderType;
  }
  ...
  public async signBulkOrder(orderComponents: T[]): Promise<Order<T>[]> {
    const signer = await this._getSigner();

    const domainData = await this._getDomainData();
    const tree = getBulkOrderTree(orderComponents, this.eip712BulkOrderType);
    ...
    const orders: Order<T>[] = orderComponents.map((parameters, i) => ({
      parameters,
  ...
  public async verifyBulkOrder(orders: Order<T>[]) {
    const address = await this._getAddress();
    return this._verifyOrder(orders, address);
  }

  private _verifyOrder(orders: Order<T>[], address: string) {
    // TODO: verify order
  }
}
